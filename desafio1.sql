...
CREATE TABLE SpotifyClone.users (
  ...
  `user_name` TEXT(25) NOT NULL,
  `age` INT(11),
  `plan_id` INT(11),
  `signature_date` DATE,
  FOREIGN KEY (plan_id) REFERENCES SpotifyClone.plans (plans_id);
) engine = INNODB;

CREATE TABLE SpotifyClone.albums (
  `album_id` INT(11) PRIMARY KEY AUTO_INCREMENT,
  `album_name` TEXT(15) NOT NULL,
  `artist_id` INT(11),
  `release_year` YEAR,
...
CREATE TABLE SpotifyClone.history (
  ...
  FOREIGN KEY (track_id) REFERENCES SpotifyClone.traks (track_id),
  FOREIGN KEY (album_id) REFERENCES SpotifyClone.albums (album_id);
) engine = INNODB;
...
INSERT INTO SpotifyClone.tracks(track_name, album_id, duration)
VALUES
  ('Soul For Us', 1, 200),
  ...
  INSERT INTO SpotifyClone.history(user_id, track_id, date)
VALUES
